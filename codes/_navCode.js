...
window.addEventListener("load", () => {
    ...
    .finished.then(() => {
        ...
        // console.log("animcaion terminada")
...
$root.addEventListener("scroll", () => {
    ...
    for (let index = 0; index < $mainSections.length; index++) {
        ...
        let navItem_index = index
        if (!(lastScrollTopPosition >= eqSectionProps.posYstart && lastScrollTopPosition < eqSectionProps.posYfinish)) {
            continue
        }
        if (lastScrollTopPosition >= eqSectionProps.posYhalf && index < $navItems_Container.length - 1) {
            navItem_index ++
        }
        $navItems_Container[navItem_index].classList.toggle("current", true)
    }
})
